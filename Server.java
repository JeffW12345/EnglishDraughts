...
class Server {
	...
	private void manageFirstClientConnected(Socket firstClientSocket)
			throws IOException {
	...
	private void manageSecondClientConnected(Socket secondClientSocket)
			throws IOException {
	...
	public void notifyClosingThread(ServerClientListenerThread clientClosing)
			throws IOException {
		otherClient(clientClosing)
				.sendMessageToClient("other_client_disonnected");
		cleanup();

	}

	public ServerClientListenerThread otherClient(
			ServerClientListenerThread otherThanThisThread) {
	...
	public ServerClientListenerThread thisClient(
			ServerClientListenerThread thisOne) {
		if (thread1 != thisOne) {
			return thread1;
		}
		return thread2;
